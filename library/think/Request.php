...
class Request
{
    ...
    public function url($complete = false)
    {
        if (!$this->url) {
            ...
            else if ($this->server('HTTP_X_REWRITE_URL')) {
    ...
    public function baseFile($domain = false)
    {
        if (!$this->baseFile) {
            ...
            if (!$this->isCli()) {
                ...
                else if (basename($this->server('PHP_SELF')) === $script_name) {
                    $url = $this->server('PHP_SELF');
                }
                else if (basename($this->server('ORIG_SCRIPT_NAME')) === $script_name) {
                    $url = $this->server('ORIG_SCRIPT_NAME');
                }
                else if (($pos = strpos($this->server('PHP_SELF'), '/' . $script_name)) !== false) {
                    $url = substr($this->server('SCRIPT_NAME'), 0, $pos) . '/' . $script_name;
                }
                else if ($this->server('DOCUMENT_ROOT') && strpos($this->server('SCRIPT_FILENAME'), $this->server('DOCUMENT_ROOT')) === 0) {
    ...
    public function method($origin = false)
    {
        if ($origin) {
            ...
            else if ($this->server('HTTP_X_HTTP_METHOD_OVERRIDE')) {
    ...
    public function file($name = '')
    {
        ...
        if (!empty($files)) {
            ...
            else if (isset($sub) && isset($array[$name][$sub])) {
    ...
    public function isSsl()
    {
        ...
        else if ($this->config['https_agent_name'] && $this->server($this->config['https_agent_name'])) {
            return true;
        }
    ...
    public function isMobile()
    {
        ...
        else if ($this->server('HTTP_USER_AGENT') && preg_match('/(blackberry|configuration\/cldc|hp |hp-|htc |htc_|htc-|iemobile|kindle|midp|mmp|motorola|mobile|nokia|opera mini|opera |Googlebot-Mobile|YahooSeeker\/M1A1-R2D2|android|iphone|ipod|mobi|palm|palmos|pocket|portalmmm|ppc;|smartphone|sonyericsson|sqh|spv|symbian|treo|up.browser|up.link|vodafone|windows ce|xda |xda_)/i', $this->server('HTTP_USER_AGENT'))) {
            return true;
        }
     ...
     * @return string
     */
    public function controller($convert = true)
    {
