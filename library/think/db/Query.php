...
class Query
{
    ...
    public function __call($method, $args)
    {
        ...
        else if (strtolower(substr($method, 0, 7)) == 'whereor') {
        ...
        else if ($this->model && method_exists($this->model, 'scope' . $method)) {
            // 动态调用命名范围
            $method = 'scope' . $method;
            array_unshift($args, $this);
    ...
    protected function lazyWrite($type, $guid, $step, $lazyTime)
    {
        ...
        else if (time() > $cache->get($guid . '_time') + $lazyTime) {
            // 删除缓存
    ...
    protected function parseWhereExp($logic, $field, $op, $condition, array $param = [], $strict = false)
    {
        ...
        if ($field instanceof Expression) {
            ...
            else if (is_string($op) && strtolower($op) == 'exp') {
    ...
    protected function parseWhereItem($logic, $field, $op, $condition, $param = [])
    {
        ...
        else if ($field && is_null($condition)) {
            if (in_array(strtoupper($op), ['NULL', 'NOTNULL', 'NOT NULL'], true)) {
                // null查询
                $where = [$field, $op, ''];
            }
            else if (in_array($op, ['=', 'eq', 'EQ', null], true)) {
                $where = [$field, 'NULL', ''];
            }
            else if (in_array($op, ['<>', 'neq', 'NEQ'], true)) {
                $where = [$field, 'NOTNULL', ''];
            }
            else {
                // 字段相等查询
        ...
        else if (in_array(strtoupper($op), ['REGEXP', 'NOT REGEXP', 'EXISTS', 'NOT EXISTS', 'NOTEXISTS'], true)) {
    ...
    public function find($data = null)
    {
        ...
        if (!empty($result)) {
            ...
            else if (!empty($this->options['json'])) {
                $this->jsonResult($result, $this->options['json'], true);
            }
        }
        else if (!empty($this->options['fail'])) {
            $this->throwNotFound($this->options);
        }
